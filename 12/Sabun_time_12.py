...
def candidates_for_pair_using_dec(pt, ct,s_table_dec):
    key_can = []
    for key in range(4**12): #(2*2)^n
        k1 = (key >> 12) & 0b111111111111 #変更
        k2 = key & 0b111111111111
        v2 = ct ^ k2
...
#k1を求める
    ...
    for k1 in range(2**12): #(2^n)
        ...
        for (i, j), dC in deltas_c.items():
            ...
            x_i = p_i ^ k1
            x_j = p_j ^ k1
            s_xi = def_S_12.S_enc(x_i) #変
            s_xj = def_S_12.S_enc(x_j) #更
            if (s_xi ^ s_xj) != dC:
                ok = False
                break
        if ok:
...
def recover_keys_from_k1_candidates(k1_list, pairs):
    ...
    for k1 in k1_list:
        p0, c0 = pairs[0]
        k2 = def_S_12.S_enc(p0 ^ k1) ^ c0 #変更
        key = (k1 << 12) | k2 #上位nビットが k1、下位nビットが k2 になる2nビット鍵を作る
        ok = True
        for p, c in pairs:
            if Encrypt_12.encrypt_pt_12(p, use_key=key) != c: #変更
                ok = False
                break
        if ok:
...
def main():
    ...
    for i in range(2**12): #(2^n)
        pairs.append((i,Encrypt_12.encrypt_pt_12(i))) #変更
    ...
    # for pt in range(8):
    #     print(f" 平文 P={zb(pt,3)} -> 暗号 C={zb(Encrypt.encrypt_pt(pt),3)}")

    # print("\n攻撃に用いる平文-暗号文ペア：")
    # for i, (p, c) in enumerate(pairs, start=1):
    #     print(f" P{i} = {zb(p,3)}  ,  C{i} = {zb(c,3)}")
    
    # # 各ペアごとの候補を列挙
    # print("\n各平文-暗号文ペア単独で成立する鍵候補（復号テーブル法）:")
    # per_pair_candidates = []
    # for idx, (p, c) in enumerate(pairs, start=1):
    #     candidate = candidates_for_pair_using_dec(p, c,def_S.S_table_dec)
    #     per_pair_candidates.append(set(candidate))
    #     print(f" ペア{idx}: 候補数 = {len(candidate)} -> {[zb(k,6) for k in sorted(candidate)]}")

    # # 全ペアの共通候補
    # common = set.intersection(*per_pair_candidates)
    # print(f"\n全てのペアを同時に満たす鍵の共通集合: {len(common)} 個")
    # for k in sorted(common):
    #     print(" 鍵 =", zb(k,6), " 上位(k1) =", zb((k>>3)&0b111,3), " 下位(k2) =", zb(k&0b111,3))
    ...
    if recovered:
        for key_val, k1, k2 in recovered:
            print(f" 鍵 = {zb(key_val,24)}  (k1={zb(k1,12)}, k2={zb(k2,12)})") #変更
    else:
