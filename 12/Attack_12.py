...
from Encrypt_12 import encrypt_pt_12  # type: ignore
import time
...
def main():
    s_table_dec = def_S_12.S_table_dec_12
    ...
    for pt in range(2**12):
        ct = encrypt_pt_12(pt)  
        # print(f"平文 {zero_bin(pt,12)} -> 暗号文 {zero_bin(ct,12)}")
        ...
        if ee == "y":
            ...
            if len(all_candidates) == 1:
                ...
                print(f" 決定候補: key={zero_bin(k,24)}  key1={zero_bin((k>>12)&0b111111111111,12)}  key2={zero_bin(k&0b111111111111,12)}")
                break
        else:
            continue
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,24)}  key1={zero_bin((k>>12)&0b111111111111,12)} key2={zero_bin(k&0b111111111111,12)}")
    else:
