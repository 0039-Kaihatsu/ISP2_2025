...
from Encrypt_10 import encrypt_pt_10  # type: ignore
import time
...
def main():
    s_table_dec = def_S_10.S_table_dec_10
    ...
    for pt in range(2**10):
        ...
        if ee == "y":
            ...
            if len(all_candidates) == 1:
                ...
                print(f" 決定候補: key={zero_bin(k,20)}  key1={zero_bin((k>>10)&0b1111111111,10)}  key2={zero_bin(k&0b1111111111,10)}")
                break
        else:
            continue
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,20)}  key1={zero_bin((k>>10)&0b1111111111,10)} key2={zero_bin(k&0b1111111111,10)}")
    else:
