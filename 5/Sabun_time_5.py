...
#k1を求める
    ...
    for k1 in range(2**5): #(2^n)
        ...
        for (i, j), dC in deltas_c.items():
            ...
            x_i = p_i ^ k1
            x_j = p_j ^ k1
            s_xi = def_S_5.S_enc(x_i) #変
            s_xj = def_S_5.S_enc(x_j) #更
            if (s_xi ^ s_xj) != dC:
                ok = False
                break
        if ok:
...
def recover_keys_from_k1_candidates(k1_list, pairs):
    ...
    for k1 in k1_list:
        p0, c0 = pairs[0]
        k2 = def_S_5.S_enc(p0 ^ k1) ^ c0 #変更
        key = (k1 << 5) | k2 #上位nビットが k1、下位nビットが k2 になる2nビット鍵を作る
        ok = True
        for p, c in pairs:
            if Encrypt_5.encrypt_pt_5(p, use_key=key) != c: #変更
                ok = False
                break
        if ok:
...
def main():
    ...
    for i in range(2**5): #(2^n)
        pairs.append((i,Encrypt_5.encrypt_pt_5(i))) #変更
    ...
    if recovered:
        for key_val, k1, k2 in recovered:
            print(f" 鍵 = {zb(key_val,8)}  (k1={zb(k1,4)}, k2={zb(k2,4)})") #変更
    else:
