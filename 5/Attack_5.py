...
from Encrypt_5 import encrypt_pt_5  # type: ignore
import time
...
def main():
    s_table_dec = def_S_5.S_table_dec_5 #変更
    ...
    for pt in range(2**5): #(2^n)
        ct = encrypt_pt_5(pt) #変更  
        ...
        if ee == "y":
            ...
            if len(all_candidates) == 1:
                ...
                print(f" 決定候補: key={zero_bin(k,4)}  key1={zero_bin((k>>2)&0b11,2)}  key2={zero_bin(k&0b11,2)}") #変更
                break
        else:
            continue
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,10)}  key1={zero_bin((k>>5)&0b11111,5)} key2={zero_bin(k&0b11111,5)}") #変更
    else:
