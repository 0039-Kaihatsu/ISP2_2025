...
# include time
import def_S
from Encrypt import encrypt_pt
import time
...
def main():
    ...
    for pt in range(2**3):
        ct = encrypt_pt(pt)  
        # print(f"平文 {zero_bin(pt,3)} -> 暗号文 {zero_bin(ct,3)}")
        ...
        if ee == "y":
            ...
            if len(all_candidates) == 1:
                ...
                print(f" 決定候補: key={zero_bin(k,6)}  key1={zero_bin((k>>3)&0b111,3)}  key2={zero_bin(k&0b111,3)}")
                break
        else:
            continue
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,6)}  key1={zero_bin((k>>3)&0b111,3)} key2={zero_bin(k&0b111,3)}")
    else:
