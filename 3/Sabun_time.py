...
#k1を求める
    ...
    for k1 in range(8):
        ...
        for (i, j), dC in deltas_c.items():
            ...
            x_i = p_i ^ k1
            x_j = p_j ^ k1
            s_xi = def_S.S_enc(x_i)
            s_xj = def_S.S_enc(x_j)
            if (s_xi ^ s_xj) != dC:
                ok = False
                break
        if ok:
...
def recover_keys_from_k1_candidates(k1_list, pairs):
    ...
    for k1 in k1_list:
        p0, c0 = pairs[0]
        k2 = def_S.S_enc(p0 ^ k1) ^ c0
        key = (k1 << 3) | k2 #上位3ビットが k1、下位3ビットが k2 になる6ビット鍵を作る
        ok = True
        for p, c in pairs:
            if Encrypt.encrypt_pt(p, use_key=key) != c:
                ok = False
                break
        if ok:
...
def main():
    ...
    for i in range(8):
        pairs.append((i,Encrypt.encrypt_pt(i)))
    ...
    if recovered:
        for key_val, k1, k2 in recovered:
            print(f" 鍵 = {zb(key_val,6)}  (k1={zb(k1,3)}, k2={zb(k2,3)})")
    else:
