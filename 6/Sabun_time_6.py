...
def candidates_for_pair_using_dec(pt, ct,s_table_dec):
    key_can = []
    for key in range(4**6): #(2*2)^n
        k1 = (key >> 6) & 0b111111 #変更
        k2 = key & 0b111111
        v2 = ct ^ k2
...
#k1を求める
    ...
    for k1 in range(2**6): #(2^n)
        ...
        for (i, j), dC in deltas_c.items():
            ...
            x_i = p_i ^ k1
            x_j = p_j ^ k1
            s_xi = def_S_6.S_enc(x_i) #変
            s_xj = def_S_6.S_enc(x_j) #更
            if (s_xi ^ s_xj) != dC:
                ok = False
                break
        if ok:
...
def recover_keys_from_k1_candidates(k1_list, pairs):
    ...
    for k1 in k1_list:
        p0, c0 = pairs[0]
        k2 = def_S_6.S_enc(p0 ^ k1) ^ c0 #変更
        key = (k1 << 6) | k2 #上位nビットが k1、下位nビットが k2 になる2nビット鍵を作る
        ok = True
        for p, c in pairs:
            if Encrypt_6.encrypt_pt_6(p, use_key=key) != c: #変更
                ok = False
                break
        if ok:
...
def main():
    ...
    for i in range(2**6): #(2^n)
        pairs.append((i,Encrypt_6.encrypt_pt_6(i))) #変更
    ...
    if recovered:
        for key_val, k1, k2 in recovered:
            print(f" 鍵 = {zb(key_val,12)}  (k1={zb(k1,6)}, k2={zb(k2,6)})") #変更
    else:
