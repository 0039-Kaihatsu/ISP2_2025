# Attack_8.py
# include time
import def_S_8  # type: ignore
from Encrypt_8 import encrypt_pt_8  # type: ignore
import time
...
def main():
    s_table_dec = def_S_8.S_table_dec_8 #変更
    ...
    for pt in range(2**8): #(2^n)
        ct = encrypt_pt_8(pt) #変更  
        ...
        if ee == "y":
            ...
            if len(all_candidates) == 1:
                ...
                print(f" 決定候補: key={zero_bin(k,16)}  key1={zero_bin((k>>8)&0b11111111,8)}  key2={zero_bin(k&0b11111111,8)}") #変更
                break
        else:
            continue
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,16)}  key1={zero_bin((k>>8)&0b11111111,8)} key2={zero_bin(k&0b11111111,8)}") #変更
    else:
