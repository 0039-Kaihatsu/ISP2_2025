...
from Encrypt_9 import encrypt_pt_9  # type: ignore
import time
...
def main():
    s_table_dec = def_S_9.S_table_dec_9 #変更
    ...
    for pt in range(2**9): #(2^n)
        ct = encrypt_pt_9(pt) #変更  
        ...
        if ee == "y":
            ...
            if len(all_candidates) == 1:
                ...
                print(f" 決定候補: key={zero_bin(k,18)}  key1={zero_bin((k>>9)&0b111111111,9)}  key2={zero_bin(k&0b111111111,9)}") #変更
                break
        else:
            continue
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,18)}  key1={zero_bin((k>>9)&0b111111111,9)} key2={zero_bin(k&0b111111111,9)}") #変更
    else:
