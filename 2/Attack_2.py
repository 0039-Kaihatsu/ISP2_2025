...
from Encrypt_2 import encrypt_pt_2  # type: ignore
import time
...
def main():
    s_table_dec = def_S_2.S_table_dec_2 #変更
    ...
    for pt in range(4): #(2^n)
        ct = encrypt_pt_2(pt)  #変更
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,4)}  key1={zero_bin((k>>2)&0b11,2)} key2={zero_bin(k&0b11,2)}") #変更
    else:
