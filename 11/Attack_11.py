...
from Encrypt_11 import encrypt_pt_11  # type: ignore
import time
...
def main():
    s_table_dec = def_S_11.S_table_dec_11
    ...
    for pt in range(2**11):
        ct = encrypt_pt_11(pt)  
        # print(f"平文 {zero_bin(pt,11)} -> 暗号文 {zero_bin(ct,11)}")
        ...
        if ee == "y":
            ...
            if len(all_candidates) == 1:
                ...
                print(f" 決定候補: key={zero_bin(k,22)}  key1={zero_bin((k>>11)&0b11111111111,11)}  key2={zero_bin(k&0b11111111111,11)}")
                break
        else:
            continue
    ...
    if all_candidates:
        if len(all_candidates) > 1:
            ...
            for k in sorted(all_candidates):
                print(f" key={zero_bin(k,22)}  key1={zero_bin((k>>11)&0b11111111111,11)} key2={zero_bin(k&0b11111111111,11)}")
    else:
